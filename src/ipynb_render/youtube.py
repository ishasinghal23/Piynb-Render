...
from ipynb_render.custom_exception import InvalidURLException
from ipynb_render.logger import logger
from py_youtube import Data

@ensure_annotations
def get_time_info(URL: str) -> int:
    def _verify_vid_id_len(vid_id, __expected_len=11):
        len_of_vid = len(vid_id)
        if len_of_vid != __expected_len:
            raise InvalidURLException(
                f"Invalid video id with length: {len_of_vid}, expected: {__expected_len}"
            )
    try:
        split_val = URL.split("=")
        if "watch" in URL:
